...
**Vectors v0.3.0 · 12/12 topics verified · 34 conformance cases · 3 themes**
...
![release-select](docs/media/release-select.png)
...
| ![themes](docs/media/themes-dark.png) | [**Three themes over one token layer**](docs/guide/themes.md) — Colour is a token, never a literal. | ✓ |
...
| TTC p95, chord | 0.80 ms | 33 | ≤ 16 ms |
...
`dark` · `light` · `contrast` — palette swaps over one token layer. Every text pair is
...
- release-select: ffmpeg not present, so this page shows a still rather than motion
